...
class TestBasicAllowance(object):
    ...
    @pytest.mark.skipif(sys.version_info <= (3, 3),
                        reason="requires Python 3.3 or higher")
...
class TestWithArgs(object):
    ...
    def test_raises_if_arguments_were_specified_but_not_provided_when_called(
            self):
        subject = InstanceDouble('dobles.testing.User')
...
class TestTwice(object):
    ...
    def test_fails_when_called_three_times(self):
        ...
        assert re.match(
            ...
            r" \(.*dobles/test/allow_test.py:\d+\)", str(e.value))
...
class TestExactly(object):
    ...
    def test_raises_if_called_with_negative_value(self):
        ...
        assert re.match(r"exactly requires one positive integer argument",
                        str(e.value))
...
class TestAtLeast(object):
    ...
    def test_raises_if_called_with_negative_value(self):
        ...
        assert re.match(r"at_least requires one positive integer argument",
                        str(e.value))
